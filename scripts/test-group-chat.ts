...
const { actOnGroupFinance, createGroupFinanceEventNotice, createGroupFinanceInteraction } = await import('../src/services/groupFinance.ts')
...
assert.match(payload[0].content, /用户只是其中一名普通成员/)
assert.match(payload[0].content, /以下内容共同构成一个正在持续发生的真实多人群聊/)
assert.match(payload[0].content, /每个 <member_context> 只属于其 id 对应的成员/)
assert.match(payload[0].content, /display_name="阿岚"/)
assert.match(payload[0].content, /当前角色是阿岚/)
assert.match(payload[0].content, /当前角色是白露/)
assert.doesNotMatch(payload[0].content, /群聊协调器|当前系统执行者|你正在(?:参与|呈现)一个真实/)
assert.doesNotMatch(payload[0].content, /你是(?:阿岚|白露)/)
...
const packetInteraction = createGroupFinanceInteraction(configured, { feature: 'packet_lucky', creatorId: 'a', creatorName: '阿岚', targetIds: [], amountCents: 20000, count: 1, remark: '恭喜发财' })
const packetClaim = actOnGroupFinance(configured, { interactionId: packetInteraction.id, actorId: 'b', action: 'claim' })
assert.equal(packetClaim.ok, true)
if (!packetClaim.ok) throw new Error('红包领取测试失败')
const packetNotice = createGroupFinanceEventNotice({ interaction: packetClaim.interaction, allocation: packetClaim.allocation, action: 'claim', actorId: 'b', actorName: '白露', createdAt: 123 })
assert.equal(packetNotice.content, '白露领取了阿岚发出的红包')
assert.equal(packetNotice.event.amountCents, 20000)
assert.equal(packetNotice.event.createdAt, 123)
const financeNoticeGroup = normalizeGroupChat({ ...configured, id: 'finance-notice-group', messages: [{ id: 123, timestamp: 123, type: 'system', messageType: 'group_finance_event', content: packetNotice.content, financeEvent: packetNotice.event, financialRef: { interactionId: packetInteraction.id } }] })
payload = await buildGroupChatMessages(financeNoticeGroup, contacts, { name: '小满' })
assert.ok(payload.some(item => item.role === 'system' && item.content === '白露领取了阿岚发出的红包'))
...
assert.deepEqual(getAtAllUsage(managedGroup, 'a'), { count: 0, limit: 10, remaining: 10, canMentionAll: true })
for (let index = 0; index < 10; index++) consumeAtAll(managedGroup, 'a')
assert.equal(getAtAllUsage(managedGroup, 'a').remaining, 0)
assert.throws(() => consumeAtAll(managedGroup, 'a'), /次数已用完/)

groupManagementService.removeMember(managedGroup, 'a', 'c', '暂时移出')
assert.equal(managedGroup.removedMembers?.c?.source, 'kicked')
const rejoinRequest = groupManagementService.requestRejoin(managedGroup, 'c', '我会遵守群规')
assert.equal(rejoinRequest.kind, 'rejoin_application')
groupManagementService.reviewRejoinApplication(managedGroup, 'user', rejoinRequest.id, true)
assert.equal(managedGroup.memberIds.includes('c'), true)
groupManagementService.leaveGroup(managedGroup, 'c', '先冷静一下')
const invitation = groupManagementService.inviteFormerMember(managedGroup, 'user', 'c', '欢迎回来')
groupManagementService.respondToInvitation(managedGroup, 'c', invitation.id, true)
assert.equal(managedGroup.memberIds.includes('c'), true)
groupManagementService.setMemberSpecialTitle(managedGroup, 'user', 'b', '气氛担当')
assert.equal(managedGroup.memberSpecialTitles?.b, '气氛担当')
