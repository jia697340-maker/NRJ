...
export type SocialGenerationField = 'nickname' | 'socialId' | 'signature' | 'moments'
...
const recentChatText = async (chat: any, allowDetails: boolean) => {
  if (!allowDetails) {
    return buildMemoryPacket(chat, '人物关系、最近经历、情绪与相处氛围', chat.memoryTokenBudget)
  }
...
export async function generateSocialProfileFields(options: {
  ...
  const chatContext = options.includeRecentChat ? await recentChatText(chat, options.allowChatDetails) : ''
  const oldMoments = options.existingMoments.slice(0, 12).map(item => item.content).filter(Boolean).join('\n')
  const system = `你是一名克制、细腻的角色社交资料编辑。请根据角色人设生成用户指定的字段，只返回一个 JSON 对象，禁止 Markdown、解释和未请求字段。\n角色原名：${chat.realName || chat.name}\n角色人设：${String(chat.persona || '').slice(0, 8000)}\n当前资料：${JSON.stringify({ nickname: profile.nickname, socialId: profile.socialId, signature: profile.signature })}\nsocialId 只能包含英文字母、数字、下划线和短横线，长度 4～20。网名和签名要自然，避免模板腔。朋友圈应像同一个真实的人在不同时间发布，长短有变化，避免每条都抒情、都提到用户或重复句式。即使参考聊天，也不得泄露密码、地址、联系方式等隐私。\n已有朋友圈（用于去重）：\n${oldMoments || '无'}\n${chatContext ? `聊天参考：\n${chatContext}\n${options.allowChatDetails ? '允许谨慎提及非敏感聊天细节。' : '只吸收情绪与关系氛围，不得复述具体私聊内容。'}` : '不要参考最近聊天。'}\n仅生成这些字段：${requested}。`
