...
export interface ForumComment { id: string; postId: string; author: ForumUser; authorAccountId: string; parentId?: string; rootCommentId?: string; replyToCommentId?: string; depth?: number; content: string; media?: ForumMediaItem[]; likeCount: number; isLiked?: boolean; createdAt: number | string; replyToUser?: { id: string; name: string }; replies?: ForumComment[]; source?: 'user' | 'generated'; generationBatchId?: string; communityResponseForCommentId?: string; communityResponseRequestId?: string; revealAt?: number; availabilityText?: string; pendingReply?: boolean }
...
export interface ForumNotification { id: string; accountId: string; type: 'like' | 'comment' | 'reply' | 'mention' | 'follow' | 'follow-request' | 'friend-request' | 'dm' | 'message-request' | 'group-invite' | 'circle-request' | 'management' | 'lottery' | 'poll' | 'event' | 'worldbook-update' | 'recognition'; actorAccountId?: string; entityId?: string; text: string; readAt?: number; createdAt: number }
