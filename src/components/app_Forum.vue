...
const refreshCircleId=ref<string|undefined>()
const characterUsers=computed(()=>forum.forumUsers.value.filter(user=>user.lifecycle==='character'))
const openGenerator=(circleId?:string)=>{forum.error.value='';refreshCircleId.value=circleId;refreshVisible.value=true}
...
<template>
  <div class="forum-app-container" :class="{'is-dark':globalSettings.darkMode,'is-light':!globalSettings.darkMode}">
    ...
    <template v-else-if="forum.currentForumUser.value">
      ...
      <ForumRefreshModal :visible="refreshVisible" :books="worldBooks.filter(book=>book.type==='book'&&book.enabled)" :initial-book-ids="forum.snapshot.value.settings.refreshWorldBookIds||[]" :initial-circle-id="refreshCircleId" :circles="forum.circles.value" :characters="characterUsers" :busy="forum.busy.value" :progress="forum.generationProgress.value" :error="forum.error.value" @close="refreshVisible=false" @confirm="runGeneration"/>
