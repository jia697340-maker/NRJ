  ...
  <div class="forum-app-container" :class="{'is-dark':globalSettings.darkMode,'is-light':!globalSettings.darkMode}">
    ...
    <template v-else-if="forum.currentForumUser.value">
      <main class="forum-main-viewport">
        ...
        <template v-else>
          <ForumFeedView v-if="forum.activeTab.value==='feed'" :posts="forum.posts.value" :mode="forum.feedMode.value" :busy="forum.busy.value" :error="forum.error.value" @back="emit('close')" @open-search="forum.pushRoute({name:'search'})" @generate="openGenerator()" @change-mode="mode=>forum.feedMode.value=mode" @shown="forum.recordFeedShown" @delete-posts="forum.deletePosts" @toggle-pin="id=>forum.togglePinPost(id)" @toggle-keep="id=>forum.toggleKeepPost(id)" @move-post="(id,dir)=>forum.movePost(id,dir)" @click-post="openPost" @click-user="openUser" @like="p=>forum.toggleLikePost(p.id)" @bookmark="p=>forum.toggleBookmarkPost(p.id)"/>
