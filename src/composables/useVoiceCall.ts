...
你现在需要扮演【${charName}】，正在处理一个是否接听语音电话的决策。
...
- 挂断：返回 {"decision": "reject"}`
  ...
  const checkAndGenerateTempSummary = async (callMessages: any[]) => {
    ...
    if (unsummarizedMsgs.length >= threshold) {
      ...
      let prompt = `请你作为一个总结助手，对以下这部分语音通话记录进行简明扼要的提要总结。
要求：
1. 提炼出关键讨论点和当前进展。
2. 必须以第三人称客观视角书写。
3. 字数控制在50-150字以内。`
  ...
  const generateFinalCallSummary = async (callMessages: any[], charName: string, userName: string) => {
    ...
    let prompt = `你是一个专业的对话总结助手。刚刚完成了一段语音通话，请根据提供的【前半段通话提要】(如果有) 以及【通话结尾的对话明细】，为本次完整的通话生成一份第三人称的档案总结。
这份总结将被存入长期记忆库中。
