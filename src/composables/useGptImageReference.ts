...
export interface GptReferenceGroup {
  id: string
  name: string
  description: string
  imageIds: string[]
...
const referenceImages = ref<GptReferenceImage[]>([])
const referenceGroups = ref<GptReferenceGroup[]>([])
const isLoaded = ref(false)

const store = localforage.createInstance({
  name: 'app_gpt_image_references',
  storeName: 'reference_data'
})

const clone = <T>(value: T): T => JSON.parse(JSON.stringify(value))

export function useGptImageReference() {
  const loadData = async () => {
    if (isLoaded.value) return
    try {
      referenceImages.value = await store.getItem<GptReferenceImage[]>('images') || []
      referenceGroups.value = await store.getItem<GptReferenceGroup[]>('groups') || []
    } finally {
      isLoaded.value = true
    }
  }
  ...
  const updateGroup = async (group: GptReferenceGroup) => {
