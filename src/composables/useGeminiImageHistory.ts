...
export interface GeminiImageHistoryItem {
  id: string
  timestamp: number
  params: any
  imageBlob?: Blob
...
export interface GeminiImageHistoryMeta {
  id: string
  timestamp: number
  params: any
}
...
const historyStore = localforage.createInstance({
  name: 'app_gemini_image_history',
  storeName: 'history_items'
})

const historyItems = ref<GeminiImageHistoryMeta[]>([])

const loadHistoryList = async () => {
  const items: GeminiImageHistoryMeta[] = []
  for (const key of await historyStore.keys()) {
    const item = await historyStore.getItem<GeminiImageHistoryItem>(key)
...
const addHistoryItem = async (params: any, image: string) => {
  ...
  const item: GeminiImageHistoryItem = {
    id,
    timestamp: Date.now(),
    params: JSON.parse(JSON.stringify(params))
  }
  try {
    item.imageBlob = await (await fetch(image)).blob()
  } catch {
    item.imageUrl = image
  }
...
const getHistoryImageUrl = async (id: string) => {
  const item = await historyStore.getItem<GeminiImageHistoryItem>(id)
...
export function useGeminiImageHistory() {
